...
from vision_transformer import vit_tiny_patch16_224, vit_small_patch16_224, vit_base_patch16_224, vit_large_patch16_224, vit_tiny_patch16_384, vit_small_patch16_384, vit_base_patch16_384, vit_large_patch16_384
...
def get_args_parser():
    ...
    parser.add_argument('--gpu', default=None, type=int,
                        help='GPU id to use.')
    parser.add_argument('--optimizer', default="Lion", type=str,
...
use_amp = True
# print("I am using amp.") if use_amp else print("I am NOT using amp")
from torch.cuda.amp import autocast, GradScaler
# scaler = GradScaler(enabled=use_amp, init_scale=8192, growth_interval=200)  # 65536->8192, 2000->200
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.arch == "vit_tiny_patch16_224":
    ...
    elif args.arch == "vit_tiny_patch16_384":
        model = vit_tiny_patch16_384(pretrained=True)
    elif args.arch == "vit_small_patch16_384":
        model = vit_small_patch16_384(pretrained=True)
    elif args.arch == "vit_base_patch16_384":
        model = vit_base_patch16_384(pretrained=True)
    elif args.arch == "vit_large_patch16_384":
    ...
    if args.resume:
            lr_func = lambda step: 0.5 * (math.cos(step / args.epochs * math.pi) + 1)  # no warmup as it has been done  # when resuming, step actually becomes 0. 
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            ...
            if args.gpu is not None:
                ...
                best_acc1 = best_acc1 if isinstance(best_acc1, float) else best_acc1.to(args.gpu)
            model.load_state_dict(checkpoint['state_dict'])
            optimizer.load_state_dict(checkpoint['optimizer'])
            scheduler.load_state_dict(checkpoint['scheduler'])
            print(f"Steps gone:{scheduler._step_count}.")
    ...
    if args.dummy:
        ...
        # ])
        # transform_complex = build_transform()
...
def train(train_loader, model, criterion, optimizer, epoch, device, args):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        if i % args.print_freq == 0 and (args.gpu is None or args.gpu == 0):
    ...
    epoch_logger({
        ...
        "top1": float(top1.avg),
        "top5": float(top5.avg),
    }) if args.gpu is None or args.gpu == 0 else None
...
"""
We import json and record the training and validation information per epoch.
We add use_amp to enable or disable amp.
We add gradient clipping.
"""